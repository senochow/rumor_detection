...
def load_json_file(filename, vocab, label):
    revs = []
    with open(filename) as f:
        num = 0
        for line in f:
            fold, content = line.rstrip().split('\t')
            ...
            for word in ori_rev.split():
                cnt += 1
                vocab[word] += 1
            datum = {"y":label, "text": ori_rev, "num_words": cnt, "split": int(fold)}
            revs.append(datum)
            num += 1
    return revs
...
def build_data_cv(data_folder, cv=10, clean_string=True):
    ...
    rumor_datas = load_json_file(pos_file, vocab, 1)
    normal_datas = load_json_file(neg_file, vocab, 0)
...
if __name__=="__main__":    
    ...
    data_folder = ["data/rumor_17.txt","data/normal_17.txt"]
    print "loading data...",        
