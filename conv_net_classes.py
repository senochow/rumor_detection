...
class LogisticRegression(object):
    ...
    def predict(self, new_data):
        p_y_given_x = T.nnet.softmax(T.dot(new_data, self.W) + self.b)
        y_pred = T.argmax(p_y_given_x, axis=1)
        return y_pred
...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), non_linear="tanh"):
        ...
        :param filter_shape: (number of filters, num input feature maps(text is 1),
...
class LeNetConvPoolLayerLoadParam(object):
    """Pool Layer of a convolutional network """

    def __init__(self, rng, input, param_w, param_b, filter_shape, image_shape, poolsize=(2, 2), non_linear="tanh"):
        """
        Allocate a LeNetConvPoolLayer with shared variable internal parameters.
